...
import { ArrowLeft, Check, Instagram } from "lucide-react-native";
...
export default function QuestionReferral() {
    ...
    const handleSelect = async (label) => {
        setSelected(label);
        try {
            await AsyncStorage.setItem("onboarding_referral_source", label);
        } catch (e) {
            console.error(e);
        }
        setTimeout(() => {
            router.push("/onboarding/question-info-prefs");
        }, 300);
    };
        ...
        <View style={{ flex: 1, backgroundColor: "#f2f5f3" }}>
            ...
            <View
                style={{
                    ...
                    borderRadius: 110, opacity: 0.6,
                }}
            />
            <View
                style={{
                    position: "absolute", bottom: 200, right: -70,
            ...
            >
                {SOURCES.map((source) => {
                    const isSelected = selected === source.label;
                    return (
                        <Pressable
                            key={source.label}
                            onPress={() => handleSelect(source.label)}
                            style={{
                        ...
                        >
                            ...
                            >
                                {isSelected ? (
                                    ...
                                    <Text style={{ fontSize: 18 }}>{source.emoji}</Text>
                                )}
                            </View>
                            <Text
                                style={{
                                    fontSize: 15,
                                    fontFamily: isSelected ? "Rubik_700Bold" : "Rubik_500Medium",
